          ...
          <div ref={menuRef} className="relative">
            ...
            {!isDeleted && (
              <button
                ...
                className={`absolute top-1 ${isMine ? '-left-7' : '-right-7'} opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-full hover:bg-gray-200 text-gray-500 text-xs`}
              >
                ⋯
              </button>
            )}
            ...
            <AnimatePresence>
              {menuOpen && (
                <motion.div
                  ...
                  className={`absolute z-50 bottom-full mb-1 ${isMine ? 'right-0' : 'left-0'} bg-white rounded-xl shadow-xl border border-gray-100 py-1 min-w-[160px]`}
                >
            ...
            <AnimatePresence>
              {reactionPickerOpen && (
                <motion.div
                  ...
                  transition={{ duration: 0.12 }}
                  className={`absolute z-50 bottom-full mb-1 ${isMine ? 'right-0' : 'left-0'} bg-white rounded-2xl shadow-xl border border-gray-100 px-2 py-1.5 flex gap-1`}
                >
    ...
    >
      ...
      >
        <div className="p-4 border-b border-gray-100">
          <h3 className="font-bold text-gray-800 text-lg">Forward Message</h3>
          <p className="text-sm text-gray-500 mt-0.5 truncate">"{message?.content?.slice(0, 50)}"</p>
        </div>
        ...
        <div className="max-h-72 overflow-y-auto p-2">
          {conversations.map((conv) => (
            ...
            >
              ...
              <span className="text-sm font-medium text-gray-700 truncate">{getConvName(conv)}</span>
            </label>
          ))}
        </div>
        ...
        <div className="p-4 border-t border-gray-100 flex gap-2">
          <button
            onClick={onClose}
            className="flex-1 py-2 rounded-xl border border-gray-200 text-gray-600 font-medium text-sm hover:bg-gray-50 transition"
          >
            Cancel
          </button>
          <button
    ...
    >
      <div className="p-3">
          ...
          <div className="mt-2 max-h-52 overflow-y-auto space-y-1">
            {results.map((msg) => (
              <button
                ...
                className="w-full text-left px-3 py-2 rounded-lg hover:bg-white border border-transparent hover:border-gray-200 transition text-sm"
              >
                <p className="font-medium text-gray-700 text-xs">{msg.senderId?.name}</p>
        ...
        {query.length > 0 && results.length === 0 && !loading && (
          <p className="text-center text-xs text-gray-400 mt-2">No messages found</p>
        )}
      </div>
    </motion.div>
  );
    ...
    >
      ...
      <div className="px-4 py-3 border-b border-gray-200 bg-white flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold text-gray-800 text-sm">{getDisplayName(currentConversation)}</h2>
        ...
        <button
          ...
          className={`p-2 rounded-lg text-sm transition ${searchOpen ? 'bg-blue-100 text-blue-600' : 'hover:bg-gray-100 text-gray-500'}`}
        >
          🔍
        </button>
      </div>
