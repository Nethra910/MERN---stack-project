...
// Menu items split into two groups so GlobalSearch slots between them
const topMenuItems = [
...
const NavItem = ({ item, totalUnread }) => {
...
export default function ModernSidebar() {
  ...
  return (
    <div className="w-64 bg-white dark:bg-dark-card border-r border-gray-200 dark:border-dark-border flex flex-col h-screen transition-colors duration-200">
      ...
      {/* ─── User profile ─────────────────────────────── */}
      <div className="px-3 py-4 border-b border-gray-100 dark:border-dark-border">
        <div
        ...
        <AnimatePresence>
          {showProfile && (
            <motion.div
              ...
              className="mt-1 px-2 py-2 bg-gray-50 dark:bg-dark-hover rounded-lg text-xs text-gray-600 dark:text-dark-muted overflow-hidden"
            >
      ...
      <nav className="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto">
        ...
        <p className="text-[10px] font-semibold text-gray-400 dark:text-dark-muted uppercase tracking-wider px-3 mb-2">
          Main Menu
        </p>

        {/* Top items: Home + Messages */}
        {topMenuItems.map(item => (
          <NavItem key={item.id} item={item} totalUnread={totalUnread} />
        ))}

        {/* ── GlobalSearch slotted between Messages & Friends ── */}
        <div className="px-1 py-2">
          <GlobalSearch />
        </div>

        {/* Bottom items: Friends → Settings */}
      ...
      {/* ─── Bell + Logout ────────────────────────────── */}
      <div className="px-3 pb-4 border-t border-gray-200 dark:border-dark-border pt-3 space-y-2">
        ...
        <div ref={requestBellRef} className="relative">
          ...
          >
            <div className="relative">
              <Bell className="w-5 h-5 text-gray-400 dark:text-dark-muted" />
              {pendingCount > 0 && (
                <motion.span
            ...
            <span className="font-medium text-sm">Chat Requests</span>
            {pendingCount > 0 && (
        ...
        <p className="text-xs text-gray-400 dark:text-dark-muted text-center">MERN Chat v1.0.0</p>
      </div>
    </div>
  );
}